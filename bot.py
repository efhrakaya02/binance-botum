...
# BTC/XAU excluded | MAX 2 positions | 10 USDT margin | 2-5x
...
TF1,TF5,TF15,TF1H="1m","5m","15m","1h"
SCAN_INTERVAL=20; MONITOR_INTERVAL=1.0; OHLCV_CACHE_SECONDS=10
POOL_LIMIT=50; MAX_POSITIONS=2; MARGIN=10.0; MIN_LEV=2; MAX_LEV=5
MIN_SCORE=72; ENTRY_SCORE=76
...
TRAIL_BASE=1.55; TRAIL_ACCEL=1.75; TRAIL_WEAK=1.05
...
# ---------------- PRICE ACTION ENGINE ----------------
def pa_engine(side,d1,d5,d15,d1h):
    s1h,_,_=structure(d1h); s15,_,_=structure(d15); s5,_,_=structure(d5)
    score=0; rs=[]; ws=[]
    wanted="BULLISH" if side=="LONG" else "BEARISH"
    if s1h==wanted: score+=18; rs.append(f"1H {wanted} HH/HL/LL/LH structure")
    elif s1h=="RANGE": score+=7; rs.append("1H range/breakout context")
    else: ws.append("1H opposing structure")
    if s15==wanted: score+=18; rs.append(f"15M {wanted} structure")
    elif s15=="RANGE": score+=8; rs.append("15M range")
    else: ws.append("15M opposing structure")
    b,bl=breakout(d5,side); r,rl=retest(d5,side); tr,tq=trigger(d1)
    if b: score+=16; rs.append(f"5M fresh breakout @ {bl:.8g}")
    if r: score+=14; rs.append(f"5M breakout-retest held @ {rl:.8g}")
    if s5==wanted: score+=8; rs.append("5M structure aligned")
    if tr: score+=12; rs.append(f"1M trigger quality {tq:.0f}/100")
    pos=move_pos(d5,side)
    if b or r:
        if pos<=.98: score+=8; rs.append("breakout not excessively extended")
        else: score-=8; ws.append("breakout at extreme")
    elif .20<=pos<=.82: score+=8; rs.append("entry not at move extreme")
    elif pos>.90: score-=15; ws.append("entry too close to move extreme")
    return {"score":clamp(score,0,100),"reasons":rs,"warnings":ws,"structures":{"1m":structure(d1)[0],
            "5m":s5,"15m":s15,"1h":s1h},"breakout":b,"retest":r,"trigger":tr,"trigger_quality":tq,
            "move_position":pos}
...
def analyze(c):
    ...
    d1,d5,d15,d1h=[ohlcv(s,x,220) for x in (TF1,TF5,TF15,TF1H)]
    if any(x is None for x in (d1,d5,d15,d1h)):return None
    price=f(c["ticker"]["last"]); results=[]
    # order book is read at decision time; it is a risk gate, not the PA signal.
    ob=restbook(s)
    if ob:
        with oblock: obstate.setdefault(s,{})["book"]=ob; obstate[s]["ts"]=time.time()
    for side in ("LONG","SHORT"):
        pa=pa_engine(side,d1,d5,d15,d1h); mo=momentum_engine(side,d1,d5,d15); li=liq_engine(s,side,price)
        score=pa["score"]*.50+mo["score"]*.30+li["score"]*.20
        b=pa["breakout"]; r=pa["retest"]; wanted="BULLISH" if side=="LONG" else "BEARISH"
        structural=pa["structures"]["15m"] in (wanted,"RANGE") or b
        eligible=(score>=MIN_SCORE and pa["trigger"] and structural and
                  mo["state"] in ("STRONG_ACCELERATING","STRONG","BUILDING") and li["status"]!="BLOCK")
        cls="PRICE_DISCOVERY_EXPANSION" if b and mo["state"]=="STRONG_ACCELERATING" else "EARLY_BREAKOUT" if b or r else "NORMAL_CONTINUATION"
        results.append({"symbol":s,"side":side,"score":score,"price":price,"funding":fu,"pa":pa,"momentum":mo,
                        "liquidity":li,"eligible":eligible,"signal_class":cls,
                        "sources":sorted(c["sources"]),"ranks":c["ranks"],"ticker_percentage":c["ticker"]["percentage"]})
    results.sort(key=lambda x:x["score"],reverse=True)
    if not results or not results[0]["eligible"] or results[0]["score"]<results[1]["score"]+5:return None
    return results[0]

def decision(sig):
    pa=sig["pa"]; mo=sig["momentum"]; li=sig.get("liquidity_final",sig["liquidity"])
    return {"signal_class":sig["signal_class"],"side":sig["side"],"score":round(sig["score"],2),
            ...
            "liquidity":li["status"],"liquidity_risk":li["risk"],
            "reasons":pa["reasons"][:6]+mo["reasons"][:5]+li["reasons"][:4],
            "warnings":pa["warnings"][:3]+mo["warnings"][:3]+li["warnings"][:4]}
...
def final(sig):
    ...
    if li["status"]=="WARNING" and sig["score"]<84:return False,"order-flow warning"
    try:
        t=ex.fetch_ticker(s); p=f(t.get("last")); move=abs(pct(sig["price"],p))
        if p<=0 or move>1.20:return False,f"price moved {move:.2f}%"
...
def openpos(sig,lev):
    ...
    log.info("ENTRY | %s %s | price %.8g | %.1fx | score %.1f | %s",s,side,entry,lev,sig["score"],sig["signal_class"])
    log.info("DECISION | %s", " | ".join(pos["decision_summary"]["reasons"]))
    return pos
...
def protection(p,price,a,mstate):
    r,_=pnl(p,price); e=p["entry_price"]; side=p["side"]; a=a or abs(price-e)*.02
    stop=e-INITIAL_STOP_ATR*a if side=="LONG" else e+INITIAL_STOP_ATR*a; stage="INITIAL"
    if r>=BE_TRIGGER:
        x=e*(1+BE_LOCK/100/p["leverage"]) if side=="LONG" else e*(1-BE_LOCK/100/p["leverage"])
        stop=max(stop,x) if side=="LONG" else min(stop,x); stage="BREAKEVEN"
    locked=None
    for trig,lk in LOCKS:
        if r>=trig: locked=lk; stage=f"LOCK_{trig:g}"
    if locked is not None:
        x=e*(1+locked/100/p["leverage"]) if side=="LONG" else e*(1-locked/100/p["leverage"])
        stop=max(stop,x) if side=="LONG" else min(stop,x)
    if r>=BE_TRIGGER:
        mult=TRAIL_ACCEL if mstate=="STRONG_ACCELERATING" else TRAIL_WEAK if mstate=="WEAKENING" else TRAIL_BASE
        if r>=25: mult=max(mult,1.60)
        if r>=50: mult=max(mult,1.75)
        x=p["peak_price"]-mult*a if side=="LONG" else p["peak_price"]+mult*a
        stop=max(stop,x) if side=="LONG" else min(stop,x)
    age=(time.time()-p["opened_ts"])/60
    action="TIME_EXIT" if age>=TIME_EXIT and p["max_roi"]<.60 else "HOLD"
    if age>=TIME_WARN and r>0 and mstate=="WEAKENING":
        x=price-1.05*a if side=="LONG" else price+1.05*a
        stop=max(stop,x) if side=="LONG" else min(stop,x); stage+="_MOM_TIGHT"
        ...
        try:
            ...
            for p in ps:
                try:
                    ...
                    if price<=0:continue
                    d5=ohlcv(p["symbol"],TF5,80); d1=ohlcv(p["symbol"],TF1,80); d15=ohlcv(p["symbol"],TF15,80)
                    ...
                    with lock:
                        ...
                        st,stage,action=protection(p,price,a,mo["state"]); p["stop_price"]=st; p["stage"]=stage
...
def scan():
    ...
    log.info("POOL | GAINERS 1-50=%d | LOSERS 1-50=%d | VOLUME 1-50=%d | BTC/XAU excluded",len(a),len(b),len(v))
    ...
    for s in signals[:5]:
        log.info("CANDIDATE | %s %s | %.1f | %s | PA %.1f | MOM %.1f/%s | LIQ %.1f/%s",
                 s["symbol"],s["side"],s["score"],s["signal_class"],s["pa"]["score"],s["momentum"]["score"],
                 s["momentum"]["state"],s["liquidity"]["score"],s["liquidity"]["status"])
    for s in signals:
        if count()>=MAX_POSITIONS:break
        ok,reason=final(s)
        if not ok:continue
        lev=leverage(s["score"],s["momentum"]["state"]); openpos(s,lev)

def bot():
    log.info("BOT START | DRY_RUN=%s | POOL 1-50/1-50/1-50 | 4 engines | 3 signal classes",DRY_RUN)
    markets()
    while True:
        try:scan()
    ...
    with lock:
        ...
        for p in positions.values():
            r,pr=pnl(p,p["current_price"]); op.append({"symbol":p["symbol"],"side":p["side"],"entry_price":p["entry_price"],
                ...
                "orderflow":p["last_orderflow_status"],"decision_summary":p["decision_summary"]})
